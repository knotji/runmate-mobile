...
export function buildRecoveryTrend(
  ...
  days: 7 | 30,
  todayDate: string,
): { points: RecoveryTrendPoint[]; insight: RecoveryTrendInsight } {
  ...
  const sleepByDate = new Map(sleep.map((night) => [night.date, night]));
  ...
  for (let offset = 0; offset < days; offset++) {
    ...
    const night = sleepByDate.get(date) ?? null;
    const olderNights = sleep.filter((candidate) => candidate.date < date).slice(0, 30);
  ...
  return { points, insight: explainLatestChange(points) };
}
...
function toSleepSignals(item: LocalHistoryItem): SleepSignals {
  ...
  return {
    ...
    sleep: finite(extracted.sleepScore),
