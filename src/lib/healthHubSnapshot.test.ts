...
vi.mock('./recoveryStartupCache', () => ({ loadRecoveryContextStartupEntry: vi.fn(), loadRecoveryStartupEntry: vi.fn() }));
...
describe('Health hub snapshot', () => {
  beforeEach(() => {
    ...
    vi.mocked(loadRecoveryContextStartupEntry).mockReturnValue(null);
  ...
  it('uses honest empty states without fetching destination data', () => {
    ...
    expect(snapshot.weight).toEqual({ label: 'No recent weigh-in', tone: 'empty' });
