...
const MealUploadFlow: React.FC = () => {
  ...
  const analyze = async () => {
    ...
    try { setMeal(await analyzeMealImages(images.map((image) => image.file), mealType, note)); }
    catch (cause) { setError(cause instanceof Error ? cause.message : 'Meal Analysis Failed'); }
    finally { setLoading(false); }
  };
  const reset = () => {
    images.forEach((image) => URL.revokeObjectURL(image.url));
    setImages([]); setMeal(null); setNote(''); setError(''); setLoading(false); setSaving(false);
  ...
  const save = async () => {
    ...
    const item = createHistoryItem('meal', { ...meal, detectedFoods, mealType, mealSlot: mealType, localDate: mealDate, note: note || meal.note });
  ...
  if (meal) return <MealReview meal={meal} date={mealDate} saving={saving} error={error} onChange={setMeal} onDate={setMealDate} onBack={() => setMeal(null)} onSave={save} />;
  return <>
    <header className="upload-intro"><p>Add Meal</p><h1>Log Your Meal</h1><span>Add up to four photos of the same meal, then review the results before saving.</span></header>
    ...
    <section className="upload-section">
      ...
      <label className="upload-picker"><IonIcon icon={cameraOutline} /><strong>{images.length ? 'Add More Photos' : 'Choose Meal Photos'}</strong><span>Use different angles or include each dish</span><input type="file" multiple accept="image/*" onChange={(event) => { choose(Array.from(event.target.files ?? [])); event.target.value = ''; }} /></label>
      <label className="upload-note"><span>Add Details <em>Optional</em></span><textarea value={note} onChange={(event) => setNote(event.target.value)} maxLength={500} placeholder="Example: Shared meal, sauce on the side" /></label>
    </section>
    {error && <p className="upload-error">{error}</p>}
    <button type="button" className="upload-primary" disabled={!images.length || loading} onClick={() => void analyze()}>{loading ? <><IonSpinner name="crescent" />Reviewing Photos...</> : `Review Meal${images.length > 1 ? ` From ${images.length} Photos` : ''}`}</button>
  </>;
};

function MealReview({ meal, date, saving, error, onChange, onDate, onBack, onSave }: { meal: MealAnalysis; date: string; saving: boolean; error: string; onChange: (meal: MealAnalysis) => void; onDate: (date: string) => void; onBack: () => void; onSave: () => Promise<void> }) {
  return <section className="upload-review">
    ...
    {error && <p className="upload-error">{error}</p>}
    <div className="upload-actions"><button type="button" onClick={onBack}>Back To Photos</button><button type="button" disabled={saving || !date || !meal.detectedFoods.some((food) => food.name.trim())} onClick={() => void onSave()}>{saving ? <IonSpinner name="crescent" /> : <IonIcon icon={checkmarkCircleOutline} />}Save Meal</button></div>
  </section>;
}
