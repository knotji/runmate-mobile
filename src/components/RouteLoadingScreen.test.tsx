...
describe('RouteLoadingScreen', () => {
  ...
  it('uses matching skeletons for tab, trend, and dynamic detail routes', () => {
    ...
    expect(loadingDetailsForPath('/tabs/you')).toEqual({ title: 'You', variant: 'profile' });
    ...
    expect(loadingDetailsForPath('/tabs/settings')).toEqual({ title: 'You', variant: 'profile' });
    expect(loadingDetailsForPath('/tabs/more')).toEqual({ title: 'You', variant: 'profile' });
