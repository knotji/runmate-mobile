...
function buildFallbackPlan(goal: Goal, contextValue: unknown, today: string) {
  ...
  return { raceCountdownText: daysLeft === 0 ? 'Race Day' : `${daysLeft} days until race`, totalWeeks, currentPhase: phase, planSummary: `A conservative ${phase.toLowerCase()} plan for ${goal.raceName}, built around ${desired} training days per week.`, phases: [{ name: phase, weekRange: `1-${totalWeeks}`, focus: phaseFocus(phase), notes: 'Adjust the plan when Recovery, pain, or illness changes.' }], weeks: [{ weekNumber: 1, phase, weeklyFocus: phaseFocus(phase), targetWeeklyDistanceKm: roundHalf(alignedWeeklyPlan.reduce((sum, item) => sum + (item.distanceKm ?? 0), 0)), longRunDistanceKm: longestDistance(alignedWeeklyPlan), workouts: alignedWeeklyPlan }], safetyNotes: 'Stop or reduce the session if pain increases, illness develops, or effort is unusually high.', weeksRemaining: totalWeeks, planStartDate: today, todayWorkout: alignedWeeklyPlan[0], weeklyPlan: alignedWeeklyPlan, paceGuidance: paces, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
}
...
function enforceRaceWeek(plan:Workout[],goal:Goal,today:string,paces:ReturnType<typeof buildPaces>):Workout[]{return plan.map((item,index)=>{const date=shiftDate(today,index);const before=dateDiff(date,goal.raceDate);if(before===0)return workout(weekdayFrom(today,index),'Race Day',distanceKm(goal.raceDistance),durationSec(goal.targetTime)?Math.round(durationSec(goal.targetTime)!/60):null,targetPaceSec(goal)?`${pace(targetPaceSec(goal)!)}/km`:null,'Race effort',`${goal.raceName} · ${goal.raceDistance}. Warm up gently, then race by controlled effort and target pace.`);if(before===1)return workout(weekdayFrom(today,index),'Rest',null,null,null,'Easy breathing','Rest before race day. Prepare your kit and keep the day easy.');if(before===2&&!/rest|recovery/i.test(item.workoutType))return workout(weekdayFrom(today,index),'Shakeout Run',Math.min(item.distanceKm??3,3),Math.min(item.durationMin??25,25),paces.recovery,'Zone 1–2','Short, relaxed shakeout. Finish feeling fresher than you started.');if((before===3||before===4)&&isHardWorkout(item.workoutType))return workout(weekdayFrom(today,index),'Race Primer',Math.min(item.distanceKm??4,4),Math.min(item.durationMin??30,30),targetPaceSec(goal)?`${pace(targetPaceSec(goal)!)}/km`:null,'Controlled race effort','Easy warm-up, 3 × 1 minute at race effort with full easy recovery, then cool down.');if(before<0)return workout(weekdayFrom(today,index),'Recovery',null,20,null,'Easy breathing','Easy walking or mobility after the race.');return item;});}
