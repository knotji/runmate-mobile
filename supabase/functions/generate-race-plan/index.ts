...
function buildFallbackPlan(goal: Goal, contextValue: unknown, today: string) {
  ...
  // One non-run day becomes Strength Training instead of Rest/Recovery, mirroring
  // the AI prompt's own guidance — never in Race Week and never when safety-capped.
  const offIndexes = [1, 3, 5, 6, 2, 4].filter((index) => !runIndexes.has(index));
  const strengthIndex = !safeOnly && daysLeft > 7 && offIndexes.length > 0 ? offIndexes[0] : -1;
  const weeklyPlan: Workout[] = dayNames.map((day, index) => {
    ...
    const candidate = committed ?? fallbackWorkout(day, index, runIndexes, safeOnly, longIndex, daysLeft, phase, easyKm, longKm, paces, strengthIndex);
