...
**Today** (`RecoveryPage.tsx`) — the daily hero screen. A large circular Recovery dial with Sleep/Strain as smaller support dials (`RecoveryDialsView.tsx`), an Energy Reserve ring card, "Today's Brief" (Body Status / Main Reason / One Useful Action — one compact card, deliberately *not* split into three, per an explicit product decision), and a Sleep Plan / Tomorrow card. Wired into `useCoachContextStore`, a dedicated startup cache, background Health Connect sync, and (indirectly) the home-screen widget. The highest-risk page in the app because of that wiring — the last of the four tabs to get its WholeMate visual pass, and it turned out to already match the target design from an earlier rebrand commit, so no reskin was needed there.
